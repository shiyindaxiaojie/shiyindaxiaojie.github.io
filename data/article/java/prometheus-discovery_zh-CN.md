---
title: Prometheus 动态获取 Pod 监控数据改造
date: 2023-08-15
description: 支持动态获取 Pod 的监控数据，并优化了官方的 Dashboard，支持区分运行环境和服务名，提高线上排查效率。
tags:
  - 项目集成
  - Spring Boot
  - Prometheus
...
Spring Boot Actuator 提供了一个监控和管理生产环境的端点，可以查看应用的运行状态。由于运维人员对 Prometheus 和 Grafana 的配置不是很熟悉，使用静态 IP 来获取监控数据，并且从 Grafana 模板市场获取的 Dashboard 不能较好的区分运行环境。

# 目标

使用 Prometheus 服务发现动态获取 Pod 的监控数据，支持 K8s 运行环境筛选监控数据。
...
假设您的配置文件是 `application.yaml`，示例配置内容如下。
...
management:
  server:
    port: 9080 # 建议和 server.port 业务端口隔离，便于区分请求流量和鉴权防护
  endpoints:
    web:
      base-path: /actuator
      exposure:
...
访问 http://127.0.0.1:9080/actuator/prometheus
...
Prometheus 默认支持 K8s 服务发现，不建议您配置静态 IP。

笔者的做法是在目标监测服务 Service，配置 2 个 Label 作为匹配项，例如 `prometheus.io/port` 监控端口
和 `prometheus.io/spring-boot-actuator-enabled` 监控开关，设置为 true 表示开启采集。
...
对应的 `prometheus.yml` 配置模板，设置自动获取 Pod 服务所在的 Namespace 和 Service。
...
scrape_configs:
  - job_name: 'spring-boot-actuator'
    ...
    - role: endpoints
    relabel_configs:
    # 对应 prometheus.io/spring-boot-actuator-enabled 注解
    - source_labels: [__meta_kubernetes_service_annotation_prometheus_io_spring_boot_actuator_enabled]
      action: keep
      regex: true
...
上述代码，除了注释标记的两个注解，其它的都可以通过 K8s 环境获取。

## 集成到 Grafana 可视化

官方提供的 Spring Boot Actuator 模板很难区分服务部署在哪一套环境，需要进行调整。

![](https://cdn.jsdelivr.net/gh/shiyindaxiaojie/cdn/grafana/grafana-spring-boot-actuator.png)

我们的需求是实现 `namespace (环境)` > `application(应用名称)` > `instance(IP)` 这样的级联过滤，因此，需要进入 `Dashboard` > `Settings` > `Variables` 中修改，您可以参考图片中的内容。

![](https://cdn.jsdelivr.net/gh/shiyindaxiaojie/cdn/grafana/grafana-spring-boot-actuator-variables.png)

修改后的 Dashboard 完整代码如下（内容较长，建议直接复制粘贴到 Grafana 中即可）。
      ...
      "targets": [
        {
          ...
          "expr": "irate(log4j2_events_total{instance=\"$instance\", application=\"$application\", level=\"info\"}[5m])",
          "format": "time_series",
          "intervalFactor": 1,
          "legendFormat": "info",
      ...
      "targets": [
        {
          ...
          "expr": "irate(log4j2_events_total{instance=\"$instance\", application=\"$application\", level=\"error\"}[5m])",
          "format": "time_series",
          "intervalFactor": 1,
          "legendFormat": "error",
          "rawSql": "SELECT\n  $__time(time_column),\n  value1\nFROM\n  metric_table\nWHERE\n  $__timeFilter(time_column)\n",
      ...
      "targets": [
        {
          ...
          "expr": "irate(log4j2_events_total{instance=\"$instance\", application=\"$application\", level=\"warn\"}[5m])",
          "format": "time_series",
          "intervalFactor": 1,
          "legendFormat": "warn",
          "rawSql": "SELECT\n  $__time(time_column),\n  value1\nFROM\n  metric_table\nWHERE\n  $__timeFilter(time_column)\n",
      ...
      "targets": [
        {
          ...
          "expr": "irate(log4j2_events_total{instance=\"$instance\", application=\"$application\", level=\"debug\"}[5m])",
          "format": "time_series",
          "intervalFactor": 1,
          "legendFormat": "debug",
          "rawSql": "SELECT\n  $__time(time_column),\n  value1\nFROM\n  metric_table\nWHERE\n  $__timeFilter(time_column)\n",
      ...
      "targets": [
        {
          ...
          "expr": "irate(log4j2_events_total{instance=\"$instance\", application=\"$application\", level=\"trace\"}[5m])",
          "format": "time_series",
          "intervalFactor": 1,
          "legendFormat": "trace",
          "rawSql": "SELECT\n  $__time(time_column),\n  value1\nFROM\n  metric_table\nWHERE\n  $__timeFilter(time_column)\n",
    ...
    "list": [
      ...
      },
      {
        "current": {
          "selected": true,
          "text": "puyiwm-gateway",
          "value": "puyiwm-gateway"
        },
        "datasource": {
          "type": "prometheus",
          "uid": "uDc5hjgVk"
        },
        "definition": "label_values(jvm_memory_used_bytes{namespace=\"$namespace\"}, application)",
        "hide": 0,
        "includeAll": false,
        "label": "Application",
        "multi": false,
        ...
        "options": [],
        "query": {
          "query": "label_values(jvm_memory_used_bytes{namespace=\"$namespace\"}, application)",
          "refId": "StandardVariableQuery"
        },
        "refresh": 1,
        "regex": "",
      ...
      },
      {
        "current": {
          "selected": false,
          "text": "172.28.80.248:9080",
          "value": "172.28.80.248:9080"
        },
        "datasource": {
          "type": "prometheus",
          "uid": "uDc5hjgVk"
        },
        "definition": "label_values(jvm_memory_used_bytes{application=\"$application\",namespace=\"$namespace\"}, instance)",
        "hide": 0,
        "includeAll": false,
        "label": "Instance",
        "multi": false,
        ...
        "options": [],
        "query": {
          "query": "label_values(jvm_memory_used_bytes{application=\"$application\",namespace=\"$namespace\"}, instance)",
          "refId": "StandardVariableQuery"
        },
        "refresh": 1,
        "regex": "",
...
![](https://cdn.jsdelivr.net/gh/shiyindaxiaojie/cdn/grafana/grafana-spring-boot-actuator-modified.png)

# 总结

支持动态获取 Pod 的监控数据，并优化了官方的 Dashboard，支持区分运行环境和服务名，提高线上排查效率。